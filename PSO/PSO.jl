...
Find a global minimum of the function with random initialization
...
- `normfMin::Float64`: The value of the objective function at `xMin`.
"""

# Define the PSO function
function PSO(fun::Function,x0::Vector{Float64};ω = .5,φp = 1.5,φg = 1.5,n_particles = 30,maxEval = 10000,threshold=1e-9)
    # PSO parameters
    ...
    personal_best_scores = [norm(fun(q)) for q in positions]
    ...
    for iter in 1:maxEval
        for i in 1:n_particles
            ...
            # Evaluate fitness
            current_score = norm(fun(positions[i]))
        ...
        if global_best_score <= threshold
            println("Threshold reached at iteration $iter with score $global_best_score")
            break
        end
    end

    return (xMin= global_best_position, normfMin= global_best_score)
end
